...
function printMessage(text = "Сообщение отсутствует") {
	console.log(text)
}

printMessage();
printMessage('Hello, Мир!');
...
let guestName = 'Igor';
function getHello(guestName = 'Гость') {
	return `Привет, ${guestName}!`;
}
console.log(getHello(guestName));
...
function calculateDifference(s, m) {
	return s - m;
}
...
function calculateProduct(pam1, pam2) {
	return pam1 * pam2;
}
...
console.log(
	'Возраст пользователя ' + calculateAge(nowYear, friendYearOfBirth) + ' лет',
);
*/
...
console.log(
	'Мой возраст ' + myAge(nowYear, myYearOfBirth) + ' года',
);
...
console.log('Результат: ' + result(6, 3));
